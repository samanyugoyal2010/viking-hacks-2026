...
import { openRouterChatText } from "@/lib/openrouter-client";
import { EXPLAIN_RESEARCH_SYSTEM_PROMPT } from "@/lib/explain-research-prompts";
  ...
  try {
    ...
    const systemBody =
      documentContext.trim().length > 0
        ? `${EXPLAIN_RESEARCH_SYSTEM_PROMPT}\n\n--- Extracted archive text (may be truncated) ---\n${documentContext.trim()}`
        : `${EXPLAIN_RESEARCH_SYSTEM_PROMPT}\n\n(No archive text loaded yet; answer from the conversation only and remind the user to upload a ZIP.)`;
